...
import random
from voc import PAD_token, EOS_token, SOS_token
from data_preprocess import MAX_LENGTH
from config import *
from model_preprocess import *
import os
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input_step, last_hidden, encoder_outputs):
        # Get word embedding and pass through dropout
        embedded = self.embedding(input_step)
        embedded = self.embedding_dropout(embedded)
        rnn_output, hidden = self.gru(embedded, last_hidden)
        # Calculate attn weights from current RNN output
        attn_weights = self.attn(rnn_output, encoder_outputs)
        # Multiply weights by encoder outputs to get weighted sum vector
...
# Calculate loss over the values in the output tensor that have a 1 in the mask.
# The entries that have a 0 are padded entries.
